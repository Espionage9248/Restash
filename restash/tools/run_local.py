...
def build_payload(url: str, api_key: str | None, mode: str) -> dict:
    parsed = urlparse(url)
    ...
    if api_key:
        conn["ApiKey"] = api_key
    return {"server_connection": conn, "args": {"mode": mode}}
...
def main() -> int:
    ...
    payload = build_payload(args.url, args.api_key, args.mode)
    proc = subprocess.run(
